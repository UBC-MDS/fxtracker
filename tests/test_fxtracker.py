...
def test_pl_trend_viz():
    ...
    assert isinstance(
        output,
        alt.vegalite.v4.api.Chart), "Altair chart object should be returned."
    ...
    assert raw['encoding']['x']['field'] == 'Date', (
            'Date should be mapped to the x axis')

    assert raw['encoding']['y']['field'] == 'Percentage Change', (
            'Percentage Change should be mapped to the y axis')
...
def test_price_trend_viz():
    ...
    with pytest.raises(Exception, match="Your option of plotting"
                       " should be from 'Open', 'High', 'Low' or 'Close'"):
    ...
    with pytest.raises(Exception, match="No data exists before"
                       " 2003-12-01, please try again."):
        price_trend_viz('EURUSD', '1992-02-01', '1994-12-31', 'High')

    # Check if the start date entered is earlier than or equal to today
    with pytest.raises(Exception, match="You entered a start date later"
                       " than today, please try again."):
        price_trend_viz('EURUSD', '2023-12-31', '2024-12-31', 'High')

    # Check if the same dates are entered for start date and end date
    with pytest.raises(Exception, match="No data found from data source."
                       " Check your ticker and date."):
        price_trend_viz('EURUSD', '2013-12-31', '2013-12-31', 'High')

    output_chart = price_trend_viz(
        'EURUSD', '2018-12-01', '2022-12-01', 'High')
...
def test_fx_conversion():
    ...
    assert isinstance(fx_conversion('EUR', 'USD', 100),
                      np.float64), "Float should be returned."

    # Check input type of curr_1
    with pytest.raises(TypeError, match="Ticker of the current"
                       " currency needs to be of str type."):
        fx_conversion(1, 'USD', 100)

    # Check input type of curr_2
    with pytest.raises(TypeError, match="Ticker of the desired "
                       "currency needs to be of str type."):
        fx_conversion('EUR', 1, 100)

    # Check input type of amt
    with pytest.raises(TypeError, match="Amount of money to be "
                       "converted needs to be a number."):
        fx_conversion('EUR', 'USD', 'a')

    # Check if no arguments are passed
    with pytest.raises(TypeError,
                       match=r"^fx_conversion\(\) missing 3 required"
                       " positional arguments: 'curr1', 'curr2', and 'amt'$"):
        fx_conversion()

    # Check if curr_2 and amt arguments are not passed
    with pytest.raises(TypeError, match=r"^fx_conversion\(\) missing 2 "
                       "required positional arguments: 'curr2' and 'amt'$"):
        fx_conversion('USD')

    # Check if amt argument is not passed
    with pytest.raises(TypeError, match=r"^fx_conversion\(\) missing 1 "
                       "required positional argument: 'amt'$"):
        fx_conversion('USD', 'EUR')

    # Check if wrong ticker(s) is/are passed
    with pytest.raises(NameError,
                       match="You have entered an "
                       "invalid foreign ticker! Try again."):
