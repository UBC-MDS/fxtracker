...
def test_price_trend_viz():
    ...
    with pytest.raises(Exception,
                       match="Your option of plotting should be from 'Open', 'High', 'Low' or 'Close'"):
    ...
    assert isinstance(
        output_chart, alt.vegalite.v4.api.Chart),
    "Altair chart object should be returned."
...
def test_fx_conversion():
    ...
    with pytest.raises(TypeError,
                       match="Ticker of the current currency needs to be of str type."):
        fx_conversion(1, 'USD', 100)

    # Check input type of curr_2
    with pytest.raises(TypeError,
                       match="Ticker of the desired currency needs to be of str type."):
    ...
    with pytest.raises(TypeError,
                       match=r"^fx_conversion\(\) missing 3 required positional arguments: 'curr1', 'curr2', and 'amt'$"):
        fx_conversion()

    # Check if curr_2 and amt arguments are not passed
    with pytest.raises(TypeError,
                       match=r"^fx_conversion\(\) missing 2 required positional arguments: 'curr2' and 'amt'$"):
        fx_conversion('USD')

    # Check if amt argument is not passed
    with pytest.raises(TypeError,
                       match=r"^fx_conversion\(\) missing 1 required positional argument: 'amt'$"):
        fx_conversion('USD', 'EUR')

    # Check if wrong ticker(s) is/are passed
    with pytest.raises(NameError,
                       match="You have entered an invalid foreign ticker! Try again."):
